...
#include <string.h>
/* ============= uint8_to_uint32 ============= */
extern int uf8_decoder(int x);
extern int uf8_encoder(int x);
...
static bool test_uf8(void)
{
    ...
    for (int i = 0; i < 256; i++) {
        int fl = i;
        int32_t value = uf8_decoder(fl);
        int fl2 = uf8_encoder(value);
        ...
        if (value <= previous_value) {
            TEST_LOGGER("Order error at ");
            print_dec(fl);
            TEST_LOGGER(": value ");
            print_dec(value);
            TEST_LOGGER(" <= previous ");
...
int main(void)
{
    ...
    if (ok){
        TEST_LOGGER("  UF8 encode/decode test: PASSED\n");}
    else{
        TEST_LOGGER("  UF8 encode/decode test: FAILED\n");
    }
    TEST_LOGGER("  Cycles: ");
    print_dec((unsigned long)cycles_elapsed);
    TEST_LOGGER("  Instructions: ");
